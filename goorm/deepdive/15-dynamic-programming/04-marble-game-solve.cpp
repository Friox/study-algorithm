...
class BigNumber {
    ...
    public:
    BigNumber() {}
    BigNumber(string v) { this->v = v; }
    BigNumber operator+(const BigNumber& t) const {
        int numberLength = max(v.length(), t.v.length()), carry = 0;
        string res;
        string a = string(numberLength - v.length(), '0') + v;
        string b = string(numberLength - t.v.length(), '0') + t.v;
        for (int i = numberLength - 1; i >= 0; i--) {
            int temp = (a[i] + b[i] - 2 * '0') + carry;
            carry = temp / 10;
            res += (temp % 10) + '0';
        }
...
int main() {
    ...
    scanf("%d %d %d", &n, &m, &k);
    totalMarbles = n + m;
    vector<vector<BigNumber>> dp(k + 1, vector<BigNumber>(totalMarbles + 1));
    dp[0][n] = BigNumber("1");
    for (int i = 1; i <= k; i++) {
        for (int j = 0; j <= totalMarbles; j++) {
            for (int offset = -1; offset <= 1; offset++) {
                int idx = j + offset;
                if (idx > 0 && idx < totalMarbles) dp[i][j] = dp[i][j] + dp[i - 1][idx];
            }
            if (j == 0 || j == totalMarbles) result = result + dp[i][j];
        }
    }
    printf("%s", result.getValue().c_str());
}